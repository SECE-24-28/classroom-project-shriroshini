...
function RechargePlansIntegrated() {
  ...
  const filteredPlans = activeFilter === 'all' 
    ? plans 
    : plans.filter(plan => plan.planType?.toLowerCase() === activeFilter);

  const filters = [
    { id: 'all', label: 'All Plans', icon: <Smartphone className="w-6 h-6" /> },
    { id: 'prepaid', label: 'Prepaid', icon: <Smartphone className="w-6 h-6" /> },
    { id: 'postpaid', label: 'Postpaid', icon: <FileText className="w-6 h-6" /> },
    { id: 'topup', label: 'Topup', icon: <DollarSign className="w-6 h-6" /> },
    { id: 'data', label: 'Data Plans', icon: <BarChart3 className="w-6 h-6" /> }
  ];
